...
from src.database.models import SystemRule, MaterialRevision, Chapter
...
async def run_reflection_cycle(course_id: int, db: Session) -> dict:
    ...
    for rev in revisions:
        revision_context_list.append({
            ...
            "content_before": (rev.content_before[:300] + "...") if rev.content_before and len(rev.content_before) > 300 else rev.content_before,
            "content_after": (rev.content_after[:300] + "...") if rev.content_after and len(rev.content_after) > 300 else rev.content_after,
        })
    ...
    for cat in categories:
        ...
        existing_rules = (
            db.query(SystemRule)
            .filter(SystemRule.course_id == course_id, SystemRule.rule_category == cat)
            .all()
        )
        ...
        db.query(SystemRule).filter(SystemRule.course_id == course_id, SystemRule.rule_category == cat).delete(synchronize_session=False)
...
def get_approved_rules_context(course_id: int, db: Session, category: str) -> str:
    ...
    rules = (
        db.query(SystemRule)
        .filter(SystemRule.course_id == course_id, SystemRule.rule_category == category, SystemRule.status == "approved")
        .all()
    )
    if not rules:
        return ""
