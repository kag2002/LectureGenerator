...
from src.services.memory_service import store_episodic_revision, retrieve_episodes, episodic_collection
...
async def test_session_consolidation():
    ...
    try:
        ...
        msg2 = ChatMessage(session_id=session.id, role="assistant", content="Tôi có thể giúp bạn soạn bài giảng. Bạn muốn chương nào?")
        msg3 = ChatMessage(session_id=session.id, role="user", content="Soạn chương 3: Cây nhị phân tìm kiếm.")
        msg4 = ChatMessage(session_id=session.id, role="assistant", content="Dưới đây là cấu trúc slide nháp cho chương 3...")
        ...
        with patch("src.services.consolidation_worker.async_call_llm_json") as mock_llm_json:
            ...
            archived_msgs = db.query(ChatMessage).filter(ChatMessage.session_id == session.id, ChatMessage.is_archived == True).all()
            ...
            # Verify that a new system message was created with the summary
